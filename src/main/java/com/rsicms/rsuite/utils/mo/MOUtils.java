...
public class MOUtils {
	...
	/**
	 * Get the input stream of an MO, exposing options RSuite's
	 * ManagedObjectService does not.
	 * 
	 * @param context
	 * @param mo
	 ...
	 * @param encoding
	 * @return The MO's input stream, after applying options RSuite's API
	 *         doesn't offer.
	 * @throws RSuiteException
	 * @throws UnsupportedEncodingException
	 * @throws TransformerException
	 */
	public static InputStream getInputStream(ExecutionContext context, ManagedObject mo, boolean includeXMLDeclaration,
		...
		String str = DomUtils.serializeToString(context, elem, includeXMLDeclaration, includeDoctypeDeclaration,
				encoding);
		return new ByteArrayInputStream(str.getBytes(encoding));
	}

	/**
	 * Get a display name for the MO. Fails over to local name when display name
	 * is null.
	 * 
	 * @param mo
	 ...
	 * @throws RSuiteException
	 *             Thrown if unable to determine the given MO's qualified
	 *             element name.
	 */
	 ...
	 * 
	 * @param mo
	 * @return a display name for the MO, or an empty string if an exception is
	 *         encountered.
	 */
	 ...
	 * @param user
	 * @param caItem
	 * @return Container or null. Null return when the CA item is not a content
	 *         assembly, content assembly reference, or CANode.
	 * @throws RSuiteException
	 */
	public static ManagedObject getManagedObject(ExecutionContext context, User user, ContentAssemblyItem caItem)
			throws RSuiteException {
	 ...
	 * @param content
	 * @param encoding
	 *            Only used if the file is believed to be XML.
	 * @return Either an instance of <code>XmlObjectSource</code> or
	 *         <code>NonXmlObjectSource</code>.
	 * @throws IOException
	 *             Thrown if unable to get bytes from given file.
	 */
	public static ObjectSource getObjectSource(ExecutionContext context, String filename, File content, String encoding)
			throws IOException {
	 ...
	 *         <code>NonXmlObjectSource</code>.
	 * @throws IOException
	 *             Thrown if unable to get bytes from input stream.
	 */
	public static ObjectSource getObjectSource(ExecutionContext context, String filename, InputStream content,
	 ...
	 *             Thrown if unable to get bytes from given file.
	 */
	public static ObjectSource getObjectSource(ExecutionContext context, String filename, String content,
			String encoding) throws IOException {
		return getObjectSource(context, filename, IOUtils.toByteArray(new StringReader(content), encoding), encoding);
	}
	 ...
	 *             Thrown if unable to get bytes from given file.
	 */
	public static ObjectSource getObjectSource(ExecutionContext context, String filename, byte[] content,
			String encoding) throws IOException {
		if (context.getRSuiteServerConfiguration().isTreatAsXmlFileExtension(FilenameUtils.getExtension(filename))) {
	 ...
	 * @param objectName
	 * @param advisor
	 *            A local MO advisor to use. May be null.
	 * @return The insert options for either a new XML MO or non-XML MO.
	 */
	public static ObjectInsertOptions getObjectInsertOptions(ExecutionContext context, ObjectSource objectSource,
	 ...
	 */
	public static ObjectUpdateOptions getObjectUpdateOptions(ExecutionContext context, ObjectSource objectSource,
			String objectName, ManagedObjectAdvisor advisor) {
	 ...
	 * @param id
	 * @param includeSubMos
	 *            Submit true to check the MO's sub-MOs.
	 * @return True if the MO is checked out. When checkSubMos is true, may also
	 *         return true when a sub MO is checked out.
	 * @throws RSuiteException
	 */
	public static boolean isCheckedOut(ExecutionContext context, User user, String id, boolean includeSubMos)
			throws RSuiteException {
	...
	/**
	 * Check out the MO, if able to. If already checked out by another user, an
	 * exception is thrown. If already checked out to the specified user, no
	 * action is performed.
	 * 
	 * @param context
	 * @param user
	 * @param id
	 * @return true if this method checked the MO out; false if the MO was
	 *         already checked out to the specified user.
	 * @throws RSuiteException
	 */
	public static boolean checkout(ExecutionContext context, User user, String id) throws RSuiteException {
		ManagedObjectService moService = context.getManagedObjectService();
		if (!moService.isCheckedOut(user, id)) {
			...
			if (moService.isCheckedOutButNotByUser(user, id)) {
				throw new RSuiteException(RSuiteException.ERROR_INTERNAL_ERROR, MOUtilsMessageProperties
						.get("error.mo.checked.out.by.different.user", moService.getCheckOutInfo(id).getUserId(), id));
			}
			return false;
		}
	}
	...
	/**
	 * Get the version specifier for the version of the MO that is immediately
	 * before the current version.
	 * 
	 * @param context
	 * @param user
	 * @param id
	 * @return Version specifier for the identified MO that immediately precedes
	 *         the current version, or null if there is only one version of the
	 *         MO.
	 * @throws RSuiteException
	 */
	public static VersionSpecifier getPreviousVersionSpecifier(ExecutionContext context, User user, String id)
			throws RSuiteException {
		...
		if (vh.size() >= 2) {
			/*
			 * Use the second entry in the list of versions, as the first
	...
	/**
	 * Delete metadata from an MO by metadata name. If the metadata repeats, all
	 * metadata items with the specified metadata name will be deleted.
	 * <p>
	 ...
	 * @throws RSuiteException
	 */
	public static void deleteMetadataEntries(User user, ManagedObjectService moService, String moid, String lmdName)
			throws RSuiteException {
	 ...
	 * @throws RSuiteException
	 */
	public static ManagedObject load(ExecutionContext context, User user, String filename, InputStream is,
			String encoding, ManagedObjectAdvisor moAdvisor) throws IOException, RSuiteException {
		return load(context, user, filename, getObjectSource(context, filename, is, encoding), moAdvisor);
	}
	 ...
	 * @throws RSuiteException
	 */
	public static ManagedObject load(ExecutionContext context, User user, String filename, ObjectSource objectSource,
			ManagedObjectAdvisor moAdvisor) throws RSuiteException {
	...
	/**
	 * Apply a transform to an MO and update the same MO with the tranform's
	 * result.
	 * <p>
	 * If you don't already have a session, use the signature that accepts
	 * <code>User</code> instead of <code>Session</code>.
	 * 
	 * @param context
	 * @param session
	 *            A valid session that identifies the user to operate as. The
	 *            session's key is also passed in as an XSL parameter when
	 *            includeStandardRSuiteXslParams is true.
	 * @param mo
	 *            The managed object to check out (when not already checked
	 *            out), apply the transform to, update with the transform
	 *            result, and check back in.
	 * @param xslUri
	 *            URI of the XSL to apply.
	 * @param xslParams
	 *            Optional parameters to pass into the XSL. Null may be sent in.
	 *            Hint: List<String> parameters are received as a sequence, at
	 *            least with Saxon.
	 * @param includeStandardRSuiteXslParams
	 *            Submit true to ensure XSLT parameters that RSuite typically
	 *            provides are included herein, specifically including the base
	 *            RSuite URL and a session key.
	 * @param baseRSuiteUrl
	 *            Only used when includeStandardRSuiteXslParams is true.
	 * @param resultEncoding
	 * @param versionNote
	 *            The new MO version's note.
	 * @throws RSuiteException
	 ...
	 * @throws IOException
	 */
	public static void applyTransformAndUpdate(ExecutionContext context, Session session, ManagedObject mo, URI xslUri,
			Map<String, Object> xslParams, boolean includeStandardRSuiteXslParams, String baseRSuiteUrl,
			String resultEncoding, String versionNote)
		...
		try {
			...
			transformResult = TransformUtils.transform(context, session, mo,
					context.getXmlApiManager().getTransformer(xslUri), xslParams, includeStandardRSuiteXslParams,
					baseRSuiteUrl);
	 ...
	 * @param mo
	 * @param includeSubMos
	 *            Submit true to also apply the ACL to the MO's sub-MOs.
	 * @param acl
	 * @param opResult
	 * @param messageKey
	 * @throws RSuiteException
	 *             public static void setACL( ExecutionContext context, User
	 *             user, ManagedObject mo, boolean includeSubMos, ACL acl,
	 *             OperationResult opResult, String messageKey) throws
	 *             RSuiteException { SecurityService securityService =
	 *             context.getSecurityService(); securityService.setACL( user,
	 *             mo.getId(), acl);
	 *             opResult.addInfoMessage(LocalMessageProperties.get(
	 *             messageKey, MOUtils.getDisplayName(mo), mo.getId())); if
	 *             (includeSubMos && mo.hasChildren()) { for (ManagedObject
	 *             subMo : mo.listDescendantManagedObjects()) {
	 *             securityService.setACL( user, subMo.getId(), acl);
	 *             opResult.addInfoMessage(LocalMessageProperties.get(
	 *             messageKey, MOUtils.getDisplayName(subMo), subMo.getId())); }
	 *             } }
	 */
}
